...
## 2026-04-04 — 发布前 lint 阻塞清理

### 优化了什么

1. **移除 `.agents` 执行回执脚本中的未使用函数**
   - 清理 `.agents/scripts/check_execution_receipt.mjs` 里的死代码
   - 消除发布前 lint 噪音，避免基础设施脚本本身拖住宿主项目发布

### 为什么要做这个优化

1. 这次发布前审计里，唯一真正的阻塞项来自 lint。
2. `.agents` 基础设施脚本如果保留无意义的未使用代码，会让宿主项目在“代码已可用”的情况下仍然无法通过最后检查。
3. 基础设施本身也必须满足和宿主代码一致的整洁标准，否则会持续制造发布摩擦。
